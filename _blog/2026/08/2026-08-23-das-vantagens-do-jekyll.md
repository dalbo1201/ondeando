...
O _Liquid_ é usado em sites mais simples e modelos em _Jekyll_, principalmente para montar `blog.html` e `post.html` que são os arquivos que ficam na pasta `_layouts` e são os temas para montar, no meu site, a página principal do [BLOG](/blog) e as páginas exclusivas de cada postagem.  
...
A ideia aqui não é ensinar a usar o _Liquid_, nem eu mesmo sei ainda, estou aprendendo. Mas eu queria trazer essa maravilha porque fiquei animado demais quando terminei de montar essas páginas e vi o quão mais simples é agora atualizá-las.  
